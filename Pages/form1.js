...
export default function Form1({navigation}){
    ...
    useEffect(()=>{
        async function dataSetter(){
            setImgPath(await AsyncStorage.getItem("@Person1Img"))
            setName(await AsyncStorage.getItem("@Person1Name"))
            setDob(await AsyncStorage.getItem("@Person1DateOfBirth"))
        }
        dataSetter()
    },[])
    const saveImg = async (image) => {
        try{
            await AsyncStorage.setItem("@Person1Img", image)
        }
        catch(error){
            console.log(error)
        }
    ...
    const saveName = async (values) => {
        try{
            ...
            {
                setName(values.name)
                await AsyncStorage.setItem("@Person1Name", values.name)
            }
    ...
    const onConfirm = async (date) => {
        ...
        try {
            ...
            await AsyncStorage.setItem("@Person1DateOfBirth", dob.toString().slice(0,15))
        }
        catch (err){
            console.log(err)
        }  
